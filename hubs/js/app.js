...
function m08RenderStandardExplore(value){
  const labels=[
  ...
  function splitItem(x){
    ...
    // object title may already be clean label while text contains flow
  ...
  function renderItem(x){
    ...
    const isFlow=/연결\s*흐름|성경\s*전체\s*흐름/.test(item.title)||String(item.text).includes('→')||String(item.text).includes('↓');
...
document.addEventListener('DOMContentLoaded', ()=>{
  setTimeout(forceVerticalFlowText, 300);
});
window.addEventListener('hashchange', ()=>setTimeout(forceVerticalFlowText, 100));
document.addEventListener('click', ()=>setTimeout(forceVerticalFlowText, 100));


function m08Escape(s){
  return String(s ?? '').replace(/[&<>"']/g, m => ({
    '&':'&amp;', '<':'&lt;', '>':'&gt;', '"':'&quot;', "'":'&#39;'
  }[m]));
}
function m08HasEmoji(s){
  return /[\u{1F300}-\u{1FAFF}\u{2600}-\u{27BF}]/u.test(String(s||''));
}
function m08IconForStep(step){
  const t=String(step||'');
  if(/출애굽/.test(t)) return '🌊';
  if(/가나안|정착|약속의 땅/.test(t)) return '🏞️';
  if(/예루살렘 함락|함락|멸망/.test(t)) return '🏚️';
  if(/70년|포로|바벨론/.test(t)) return '⛓️';
  if(/고레스|칙령/.test(t)) return '📜';
  if(/스룹바벨|귀환|포로귀환/.test(t)) return '🚶';
  if(/제단/.test(t)) return '🔥';
  if(/성전|제2성전|예배/.test(t)) return '🏛️';
  if(/학개|스가랴|선지자/.test(t)) return '📣';
  if(/에스더|왕후/.test(t)) return '👑';
  if(/하만|음모/.test(t)) return '⚠️';
  if(/왕의 불면증/.test(t)) return '🌙';
  if(/부림절|구원/.test(t)) return '🎉';
  if(/율법|말씀|에스라/.test(t)) return '📖';
  if(/회개|언약 갱신/.test(t)) return '😭';
  if(/성벽|느헤미야/.test(t)) return '🧱';
  if(/공동체/.test(t)) return '👥';
  if(/그리스도|예수|메시아|십자가/.test(t)) return '✝️';
  if(/교회/.test(t)) return '⛪';
  return '🔹';
}
function m08VerticalFlowHtml(text){
  const raw=String(text||'').trim();
  const parts=raw.split(/\s*(?:→|↓|\n↓\n)\s*/).map(v=>v.trim()).filter(Boolean);
  if(parts.length<2) return m08Escape(raw).replace(/\n/g,'<br>');
  return parts.map((step,i)=>{
    const clean=step.replace(/^🔹\s*/,'').trim();
    const labeled=m08HasEmoji(clean)?clean:`${m08IconForStep(clean)} ${clean}`;
    const line=`<div class="flowStep">${m08Escape(labeled)}</div>`;
    return i===parts.length-1 ? line : line + `<div class="flowDown">↓</div>`;
  }).join('');
}
function m08RenderStandardExplore(value){
  const labels=[
  ...
  function renderItem(x){
    const item=splitItem(x);
    const isFlow=/연결\s*흐름|성경\s*전체\s*흐름/.test(item.title)||String(item.text).includes('→')||String(item.text).includes('↓');
    const body=isFlow?m08VerticalFlowHtml(item.text):m08Escape(item.text).replace(/\n/g,'<br>');
    return `<div class="exploreCard">${item.title?`<b>${m08Escape(item.title)}</b>`:''}<p>${body}</p></div>`;
  }
  return `<div class="exploreGrid">${(value||[]).map(renderItem).join('')}</div>`;
}
...
function m08ForceFlowBlocks(){
  document.querySelectorAll('.exploreCard').forEach(card=>{
    ...
    if((title.includes('연결 흐름')||title.includes('성경 전체 흐름')||text.includes('→')||text.includes('↓')) && !p.querySelector('.flowStep')){
      p.innerHTML=m08VerticalFlowHtml(text);
    }
  });
}
document.addEventListener('DOMContentLoaded',()=>setTimeout(m08ForceFlowBlocks,200));
document.addEventListener('click',()=>setTimeout(m08ForceFlowBlocks,120));
